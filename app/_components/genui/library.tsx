...
const Report = defineComponent({
  name: "Report",
  description: "Root container. Pass the ordered report blocks as its only argument.",
  props: z.object({ children: childrenSchema }),
  component: ({ props, renderNode }) => <article className="grid gap-[26px] rounded-[4px_18px_4px_4px] border border-[#c9d7d4] bg-paper p-[clamp(18px,3vw,30px)] shadow-[0_16px_44px_rgb(16_38_59_/_8%)]">{renderNode(props.children)}</article>,
});

const CaseSummary = defineComponent({
  name: "CaseSummary",
  description: "Canonical case summary from the analysis tool. Takes no arguments.",
  props: emptyProps("CaseSummary"),
  component: () => {
    const { output } = useReportContext();
...
const PriorityBanner = defineComponent({
  name: "PriorityBanner",
  description: "Canonical priority and reasons from the analysis tool. Takes no arguments.",
  props: emptyProps("PriorityBanner"),
  component: () => {
    const { output } = useReportContext();
...
const MissingInfo = defineComponent({ name: "MissingInfo", description: "Missing information from the analysis tool. Takes no arguments.", props: emptyProps("MissingInfo"), component: () => { const { output } = useReportContext(); return <Section title="不足情報">{renderItems(output.report.missingInfo, "不足情報はありません")}</Section>; } });
const ActionItems = defineComponent({ name: "ActionItems", description: "Initial actions from the analysis tool. Takes no arguments.", props: emptyProps("ActionItems"), component: () => { const { output } = useReportContext(); return <Section title="初動確認事項">{renderItems(output.report.actionItems, "確認事項はありません")}</Section>; } });

const SimilarCases = defineComponent({ name: "SimilarCases", description: "Ranked similar cases from the analysis tool. Takes no arguments.", props: emptyProps("SimilarCases"), component: () => { const { output } = useReportContext(); const result = output.report.similarCases; return <Section title="類似事例">{result.matches.length === 0 ? <p className="m-0 rounded-[10px] border border-dashed border-[#c4a75c] bg-amber-pale p-3.5 text-[0.78rem] text-[#654c1d]">十分な根拠なし</p> : <ol className="m-0 grid list-none gap-3 p-0" role="list">{result.matches.map((match, index) => <li className="rounded-[10px] border border-line bg-[#fbfdfc] p-3" key={match.case.id}><strong>{String(index + 1).padStart(2, "0")}　{match.case.id}</strong><p className="my-1 text-[0.82rem]">{match.case.summary}</p><p className="m-0 text-[0.76rem] text-ink-soft">{match.score} 点</p></li>)}</ol>}</Section>; } });
const Guides = defineComponent({ name: "Guides", description: "Ranked internal guides from the analysis tool. Takes no arguments.", props: emptyProps("Guides"), component: () => { const { output } = useReportContext(); const result = output.report.guides; return <Section title="社内初動ガイド">{result.matches.length === 0 ? <p className="m-0 rounded-[10px] border border-dashed border-[#c4a75c] bg-amber-pale p-3.5 text-[0.78rem] text-[#654c1d]">十分な根拠なし</p> : <ol className="m-0 grid list-none gap-3 p-0" role="list">{result.matches.map((match, index) => <li className="rounded-[10px] border border-line bg-[#fbfdfc] p-3" key={match.guide.id}><strong>{String(index + 1).padStart(2, "0")}　{match.guide.id}　{match.guide.title}</strong><p className="m-0 mt-1 text-[0.76rem] text-ink-soft">{match.score} 点</p></li>)}</ol>}</Section>; } });
const Experts = defineComponent({ name: "Experts", description: "Ranked internal experts from the analysis tool. Takes no arguments.", props: emptyProps("Experts"), component: () => { const { output, canRespond, onRequestConsultation } = useReportContext(); const result = output.report.experts; return <Section title="有識者候補">{result.matches.length === 0 ? <p className="m-0 rounded-[10px] border border-dashed border-[#c4a75c] bg-amber-pale p-3.5 text-[0.78rem] text-[#654c1d]">十分な根拠なし</p> : <div className="grid gap-3.5 md:grid-cols-2">{result.matches.map((match, index) => <article className="grid gap-2 rounded-[10px] border border-line bg-[#fbfdfc] p-3" key={match.expert.id}><strong>{String(index + 1).padStart(2, "0")}　{match.expert.name}</strong><p className="m-0 text-[0.76rem] text-ink-soft">{match.expert.department}・{match.score} 点</p><p className="m-0 text-[0.78rem]">{match.recommendation}</p><button className="min-h-10 rounded-lg border border-control-line bg-white px-3.5 py-2 text-[0.78rem] font-extrabold text-navy hover:border-teal hover:bg-teal-pale disabled:cursor-not-allowed disabled:opacity-[.55]" type="button" disabled={!canRespond} onClick={() => void onRequestConsultation(match.expert)}>この人への相談文を作成</button></article>)}</div>}</Section>; } });
const Escalation = defineComponent({ name: "Escalation", description: "Human escalation and the canonical follow-up question. Takes no arguments.", props: emptyProps("Escalation"), component: () => { const { output, canRespond, onFocusComposer } = useReportContext(); return <><Section title="人間・専門家の確認が必要"><div className="rounded-r-[10px] border border-[#e4d3ad] border-l-4 border-l-amber bg-amber-pale px-[18px] py-4"><ul className="m-0 grid gap-1.5 pl-5 text-[0.85rem] leading-[1.7] text-[#4d3a12]">{output.report.humanEscalation.map(item => <li key={item}>{item}</li>)}</ul><p className="mb-0 text-[0.74rem] font-bold text-[#6d5011]">最終判断と実際の連絡は、担当者または適切な専門家が行ってください。</p></div></Section><section className="flex flex-wrap items-center gap-4 rounded-xl bg-navy-deep px-5 py-[18px] text-white"><div className="min-w-[220px] flex-1"><p className="mb-1 text-[0.68rem] font-extrabold tracking-[0.08em] text-[#a9cbc7]">次に確認したいこと</p><h3 className="m-0 text-base leading-[1.6]">{output.report.followUpQuestion}</h3></div><button className="min-h-10 rounded-lg border border-control-line bg-white px-3.5 py-2 text-[0.78rem] font-extrabold text-navy hover:border-teal hover:bg-teal-pale disabled:cursor-not-allowed disabled:opacity-[.55]" type="button" disabled={!canRespond} onClick={onFocusComposer}>追加情報を入力して再分析</button></section></>; } });
const ReanalysisChanges = defineComponent({ name: "ReanalysisChanges", description: "Changes since the previous analysis. Use only for a reanalysis. Takes no arguments.", props: emptyProps("ReanalysisChanges"), component: () => { const { output } = useReportContext(); const changes = output.report.reanalysisChanges; if (!changes) return null; return <Section title="前回分析からの更新"><div className="grid gap-5 md:grid-cols-2"><div><h4 className="mt-0 mb-2.5 text-[0.82rem] text-navy">解消した不明点</h4>{renderItems(changes.resolvedUnknowns, "今回解消した不明点はありません")}</div><div><h4 className="mt-0 mb-2.5 text-[0.82rem] text-navy">新たに判明した事実</h4>{renderItems(changes.newFacts, "新たに判明した事実はありません")}</div></div></Section>; } });
...
export const library = createLibrary({
  root: "Report",
  components: [Report, CaseSummary, PriorityBanner, MissingInfo, ActionItems, SimilarCases, Guides, Experts, Escalation, ReanalysisChanges, Note],
});
