...
このドキュメントは「複雑不動産案件 初動支援アシスタント」PoC のコーディング規約を定義します。本リポジトリは、Eve単体のバックエンドではなく、Nuxt 4にEveエージェントを組み込んだフルスタックアプリケーションです。
...
Eveのコードを変更する前に、必要であれば依存関係をインストールし、`node_modules/eve/dist/docs/public/` の該当ガイドを読みます。インストール済みバージョンの配置が異なる場合は、パスやAPIを推測せず `node_modules/eve` 内の同梱ドキュメントを探します。

## PoCの境界

対象はWebチャネルとダミーJSONデータのみです。次の機能を追加または再有効化しません。

- Slack、iMessage、メールなどのメッセージチャネル
...
無効化対象のEve組み込みツールは、明示的に無効な状態を維持します。AIは法務、税務、査定価格、契約可否の最終判断を行いません。

## プロジェクト構成

```text
agent/   # Eve設定、Webチャネル、instructions、skills、tools、エージェント専用ヘルパー
app/     # Nuxt/Vueのページ、レイアウト、コンポーネント、composable、クライアント処理
server/  # Nitro API、サーバーロジック、Drizzleスキーマ、マイグレーション
shared/  # agent/app/server間で共有する型とヘルパー
tests/   # 決定的なVitestテスト
docs/    # アーキテクチャ、環境、運用ドキュメント
```

1ファイルは1つの主要な責務に集中させます。Eveツールは1ツール1ファイルとします。APIルートは薄く保ち、再利用可能なビジネスロジックや永続化処理は `server/utils/` などの適切なサーバーモジュールへ分離します。

アーキテクチャを意図的に変更しない限り、`src/` や別のfeature階層を導入しません。

## インポートパス

所有レイヤーに対応するエイリアスを使います。

- `#lib/*` などの `#*`：`package.json` により `agent/*` へ解決
- `~/`：Nuxtのappディレクトリ
- `~~/`：リポジトリルート
- `#shared/`：Nuxtが提供するsharedコード用エイリアス

同じモジュール内の近接ファイルには相対importを使用できます。`../../../` のような深い相対importは避け、適切な所有レイヤーのエイリアスを使用します。
...
| 型・Vueコンポーネント | `UpperCamelCase` |
| 真の定数 | `UPPER_SNAKE_CASE` |
| 一般ファイル | 原則kebab-case |
| Eveツール | snake_case ASCII。ファイル名がモデルに見えるツール名になる |

トップレベルでexportするヘルパーは原則として関数宣言を使います。インラインcallback、Vue composable内のclosure、フレームワーク設定のcallbackではarrow functionを使用できます。
...
Eveはauthored slotのdefault exportを解決するため、default exportを一律禁止しません。
...
- Vue SFCはVue 3 Composition APIと `<script setup lang="ts">` を使用します。
- server-only secretとDBアクセスを `app/` やpublic runtime configへ置きません。
- サーバー設定には `runtimeConfig` を使い、意図的に公開する値だけを `runtimeConfig.public` に置きます。
- API handlerは、認証、parse、domain/server logicの呼出し、HTTP responseへの変換に集中させます。
- client supplied ownership fieldを信用せず、認証済みuser IDで読み書きをscopeします。
- semantic HTMLと `REQUIREMENT.md` のアクセシビリティ要件を維持します。
- 利用者向けUI、ダミーデータ、エージェント出力は、技術識別子を除いて日本語にします。

## エラーハンドリング

HTTPと認証の境界ではフレームワーク標準のerrorを使用します。秘密値や機微な入力を漏らさず、調査に必要なcontextを残します。

`better-result` は利用可能ですが任意です。期待される型付きfailureを合成する利点がある場合に使い、すべての関数を機械的に `Result` 化しません。

`Result` を使う場合は次を守ります。

- `any` ではなくtagged/discriminated errorを使う
- 全error branchを明示的に処理する
- serialization boundaryでplainなHTTP/tool responseへ変換する
- `.match()` のみを強制せず、対応するtype guardが明確なら使用する

## セキュリティ

- secretをhardcodeまたはcommitしません。
- server secretは信頼済みのserver/runtimeコードからのみ `runtimeConfig` または `process.env` で読みます。
- secretを `runtimeConfig.public`、client bundle、log、prompt、tool output、dummy JSON、documentation exampleへ出しません。
...
内部call count、Vue componentの内部state、DB内部row構造ではなく公開結果を検証します。PlaywrightやLLM依存の非決定的CIテストは追加しません。UI変更後は利用可能な環境でmanual browser smoke testを行います。
...
- Drizzle schemaは `server/db/schema/` に定義し、repositoryのschema entry pointからexportします。
- `pnpm db:generate` と `pnpm db:migrate` でmigrationを生成・適用します。
- 通常作業でraw `drizzle-kit` commandを使用しません。
