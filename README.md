...
3. 次の3パターンから1つを選び、そのまま入力欄にコピペして送信する
   ...
   **事故物件・告知事項**
   ...
   ```
   相続した物件で以前に居住者が亡くなっていたことが分かり、売却する際に何を伝えるべきか不安がある
   ```

4. エージェントが案件要約・初動確認事項・類似案件・有識者候補を構造化カードで提示する
5. 有識者候補カードの「この人への相談文を作成」を押すと下書きが生成される。「全文をコピー」でコピーできる
6. 確認後、ユーザーメニューの「デモデータを削除」でデモデータを削除する
