...
> - 同時に実行できる分析は1ユーザーにつき1件です。前の分析が実行中・終了処理中に送信するとエラーになります。画面上部のステータスが「待機中」に戻ってから再度お試しください。
...
将来的にはRAG連携（社内ナレッジ、過去案件、案件管理システムなど外部のDBや社内ドキュメントなどから情報を検索する機能）、メッセージング（Slack連携）など新たな付加価値を与え、AIだけではなく **「AI + 人」** によるナレッジ共有基盤へ発展させることを想定しています。
また、プロダクト化にあたっては、共有インスタンスをマルチインスタンス化し、並列処理を可能とすることによる、UXの改善を測る必要性もあります。
