...
**「複雑案件の初動のスピードを上げ、案件全体の流れを整理できれば、結果として事業拡大を支えられるのではないか」**

という仮定した課題の解決を図ることを目的としています。

想定ユーザーは、**お客様から問い合わせを受けた社内の担当者**（一次受付・営業・経験の浅い担当者など）です。お客様からの直接の問い合わせに加えて、士業・葬祭・自治体などの**提携先から持ち込まれる相談の一次受付**も想定しています。また、生成された相談依頼文を受け取る**社内の有識者側**にとっても、依頼内容が定型化されることで確認の負担が減ることを狙っています。

お客様向けではなく、担当者が初動対応を組み立てるための社内向けアシスタントとして構成しています（特定の企業の業務を再現したものではありません）。
...
本PoCでは、このうち代表的な3類型（相続・共有名義 / 事故・告知事項 / 再建築不可）のダミーデータに絞って検証しています。カテゴリ外の相談でも、相談内容の構造化と初動整理そのものは機能します。
...
| ④   | 最終的には**人へ相談する必要がある**が、経緯の整理や依頼文の作成に手間がかかる |
...
以下は「**お客様から複雑な相談の問い合わせを受けた担当者が、その内容をもとに初動対応を組み立てる**」というシチュエーションを想定したフローです。
...
4. `/chat/{id}` の入力欄に相談内容を入力（またはサンプル選択時は自動反映された内容を確認）し、送信する。次の3パターンのいずれかで試すと分かりやすい
   ...
   **再建築不可・接道**
   ...
   ```
   お客様から「接道条件に懸念がある古い家屋を所有しており、活用や売却を検討したい」という問い合わせを受けた。初動として何を確認すべきか整理したい
   ```

   **相続・共有持分**

   ```
   お客様から「親から相続した空き家を兄弟で共有しているが、共有者の一人と連絡が取れない」という相談を受けた。今後の進め方を整理したい
   ```

   **事故・告知事項**（新規相談ホームの「事故・告知事項」サンプルカードから試すのが手軽で、試すには良いかと思います）
...
> **エラーが出た場合**
>
> - 本デモは共有インスタンスのため、利用回数に上限があります（全ユーザー合計 200回/日、1ユーザーあたり 50回/日・8回/分）。上限に達した旨のエラーが表示された場合は、時間をおくか日を改めてお試しください。
> - 同一拠点（同一IPアドレス）からの利用は合計 120回/時 までです。複数人で同時にお試しの場合、この上限に当たることがあります。
> - AI利用料に月間予算（20 USD）を設定しています。予算上限に達した場合、その月はデモを利用できなくなることがあります。※ 月間予算に達成したと思われる場合は、ご連絡ください。
> - 同時に実行できる分析は1つの相談スレッドにつき1件です。同じスレッドで前の分析が実行中・終了処理中に送信するとエラーになります。画面上部のステータスが「待機中」に戻ってから再度お試しください。別のスレッド・別のユーザーであれば並行して分析できます。分析がエラーで中断した場合は、多くの場合すぐに再送信できます（ごく早期の失敗のみ、最大約3分お待ちいただく場合があります）。
> - 1ユーザーが保持できる相談スレッドは最大25件です。上限に達した場合は、不要な相談を削除するか、ユーザーメニューの「デモデータを削除」を実行してください。
...
1. LLMは相談文から構造化クエリ（カテゴリ・タグ・論点・物件状態・権利）を抽出するだけ
2. 検索・採点・順位付けは [`agent/lib/domain/search.ts`](./agent/lib/domain/search.ts) の固定ルールで計算する

| ルール   | 内容                                                                              |
| -------- | --------------------------------------------------------------------------------- |
| 採点     | カテゴリ・専門分野の一致 +5点、タグ・論点・強みの一致 各+2点、物件状態・権利の一致 各+1点 |
| 足切り   | 合計3点未満の候補は「十分な根拠なし」として非表示                                 |
| 同点処理 | 類似案件・ガイドはID昇順。有識者はスコア → 関連案件数 → 社員ID昇順                |
| 表示件数 | 類似案件3件・ガイド2件・有識者3件                                                 |

採点に乱数・時刻・LLMの判断が入らないため結果は再現可能で、Vitestの決定的テストで検証できます。候補は必ずダミーデータに実在するレコードから選ばれ、「どのシグナルで何点」という根拠を提示できます。安全・緊急キーワードによる優先度判定も同じく固定のキーワード表で確定します。
...
- ダミーデータは3類型（相続・事故・再建築不可）のみ。カテゴリ外の相談（離婚に伴う売却など）では、タグ一致だけで選ばれた参考度の低い類似案件・ガイドが表示されることがあります（カテゴリ不一致の注記表示は未実装）
...
- TanStack Form（案件名・チャット入力のフォーム状態管理）
- OpenUI（`@openuidev/react-lang`。エージェント応答内のOpenUI Langを分析結果カードとして描画するGenerative UI）
...
本番展開では、**使うほど案件データが蓄積される仕組みそのものが資産になる**と考えています。以下はいずれも本PoCでは未実装の構想です。

- **案件データの蓄積**: 対応履歴が貯まるほど、類似案件・有識者提案の精度を高められる
- **失敗の先回り**: 過去案件でつまずいた点を初動確認事項へ反映し、同じ失敗を繰り返さない
- **RAG連携**: 社内ナレッジ・過去案件・案件管理システムなど、外部DBや社内ドキュメントからの情報検索
- **メッセージング連携**: Slack連携などによる有識者への相談導線の短縮

これらの積み重ねにより、AIだけではなく **「AI + 人」** によるナレッジ共有基盤へ発展させることを想定しています。
